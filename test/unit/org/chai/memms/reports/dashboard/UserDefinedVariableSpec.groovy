...
class UserDefinedVariableSpec extends UnitSpec {
     ...
     def "user defined valiable is valid"() {
          ...
          when:
           ...
           def userDefVal=new UserDefinedVariable(code:"WO_REINCIDENCE_DAYS",name_en:"Work order re-incidence period(days)",name_fr:"Work order re-incidence period(days)",currentValue:365.0).save(failOnError: true, flush:true)
             userDefVal.validate()
